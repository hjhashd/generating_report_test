...
### 6.4 最佳实践演进：增强版标题识别与安全回滚 (2026-03-13)

**状态**: ✅ 已完成 (Production & Test)
**修改文件**: `/root/zzp/langextract-main/generate_report_test/utils/zzp/import_doc_to_db.py`

#### 核心挑战：非标准样式的标题识别
用户上传的 Word 文档（如 `345.docx`）往往没有严格使用 Word 的“标题 1/2/3”样式，而是通过手动输入 `1. 概述`、`1.1 项目背景` 并加粗字号来模拟标题。原有的 `python-docx` 样式识别逻辑无法捕捉这些章节，导致识别出 0 个章节，进而生成空的报告目录。

#### 解决方案：样式识别 + 正则兜底 (Regex-Based Detection)
引入了双重识别机制，确保在样式失效时仍能通过文本模式锁定章节：
1.  **样式优先**：保留对标准 `Heading 1-9` 样式的识别。
2.  **正则兜底**：增加对以下模式的自动识别：
    *   **数字编号**：支持 `1. `、`1.1 `、`1.1.1 ` 等（包括结尾带点或不带点的情况）。
    *   **中文编号**：支持 `一、`、`二、` 等一级标题。
    *   **智能级别推断**：根据点号数量自动推断 `level`（如 `1.1.1` 识别为 3 级）。

#### 安全保障：零章节回滚 (Zero-Section Rollback)
为了防止在识别失败时产生“空壳”数据（即数据库有记录但磁盘无文件）：
- **主动拦截**：在拆分循环开始前，如果识别到的 `sections` 列表为空，立即中止处理。
- **事务回滚**：利用数据库事务 (`trans.rollback()`) 撤销已创建的 `report_name` 记录。
- **清晰反馈**：向前端返回明确的错误提示：“未能识别到文档中的章节结构，请检查文档是否使用了标准标题样式或 1. 1.1 等编号格式。”

---

## 7. 已知问题与待解决事项 (Known Issues & Pending Items)
