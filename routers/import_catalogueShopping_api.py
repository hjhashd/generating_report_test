...
from utils.zzp.import_catalogueShopping import get_specific_category_tree
...
# 请求参数模型
class ImportCatalogue(BaseModel):
    task_id: str
    status: int
    agentUserId: int
    type_name: str
...
@router.post("/Import_catalogue/")
def Import_catalogue_endpoint(report: ImportCatalogue):
    logger.info(f'接收到的参数：{report}')

    try:
        # 1. 执行查询
        # 注意：这里返回的是一个字典对象 {'reportName':..., 'chapters':...}
        result_data = get_specific_category_tree(report.type_name, report.report_name,report.catalogue_name)
