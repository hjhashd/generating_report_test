...
class DocxExtractor:
    ...
    def is_heading(self, para_elem):
        ...
        if style_elem is not None:
            ...
            if style_val.startswith('Heading') or style_val.startswith(''):
                return True, style_val
        return False, None
    ...
    def list_all_headings(self):
        ...
        for idx, child in enumerate(body):
            if child.tag == f'{{{NAMESPACES["w"]}}}p':
                ...
                if is_head:
                    text = self.get_paragraph_text(child).strip()
                    level = style.replace('Heading', '') if 'Heading' in style else '?'
                    headings.append({
                        'index': idx,
                        'level': level,
                        'style': style,
...
def list_headings(docx_path):
    ...
    with DocxExtractor(docx_path) as extractor:
        ...
        for h in headings:
            indent = "  " * (int(h['level']) - 1) if h['level'].isdigit() else ""
            print(f"{indent}[级别{h['level']}] {h['text']}")
...
if __name__ == "__main__":
    ...
    # 命令行模式
    # main()
    list_headings("generate_report/utils/zzp/word拆分/XA_证书.docx")
    extract_content("generate_report/utils/zzp/word拆分/XA_证书.docx","资质证书表及证明材料")
